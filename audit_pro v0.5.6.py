...
FIN_CHECKLIST_ITEMS = [
    ...
    ("fc_06", "Receipt of share application money & allotment disclosure — Where shares have been allotted for cash, whether cash has actually been received and is properly disclosed in the balance sheet; and where no cash has been received, the position has been disclosed clearly"),
]
...
<thead><tr style='background:#1a2a3a;color:#b0c4d4'>
  ...
  <th style='width:180px'>Comment</th><th style='width:100px'>Attachments</th>
    ...
    def _build_ifc(self, parent):
        ...
        ls_sb = tk.Scrollbar(list_out, orient="vertical", command=ls_cv.yview)
        ...
        ls_inner.bind("<Configure>",
                      lambda e, c=ls_cv: c.configure(scrollregion=c.bbox("all")))
        ...
        r_sb  = tk.Scrollbar(right, orient="vertical", command=r_cv.yview)
        ...
        r_inner.bind("<Configure>",
                     lambda e, c=r_cv: c.configure(scrollregion=c.bbox("all")))
  ...
  <tbody>{rows}</tbody>
    ...
    def _build_notebook(self):
        ...
        if self._eng.get("locked", False):
            banner = tk.Frame(self, bg="#2A1A1E", pady=8, padx=20)
            banner.pack(fill="x")
            tk.Label(banner,
    ...
    def _build_workpapers(self, parent):
        ...
        sb = tk.Scrollbar(list_out, orient="vertical", command=cv.yview)
    ...
    def _select_clause(self, key, num, name):
        ...
        att_sv = tk.Scrollbar(att_out, orient="vertical", command=att_cv.yview)
        ...
        att_list.bind("<Configure>",
            lambda e, c=att_cv: c.configure(scrollregion=c.bbox("all")))
    ...
    def _build_caro(self, parent):
        ...
        ls_inner.bind("<Configure>", lambda e, c=cv: c.configure(scrollregion=c.bbox("all")))
...
class DetailPanel(tk.Frame):
    ...
    def _build(self):
        # ── Top bar ──────────────────────────────────────────────────────
        top = tk.Frame(self, bg=C["sidebar"], pady=0)
        top.pack(fill="x")
    ...
    def _build_left_panel(self):
        ...
        av_canvas.pack()
        av_canvas.create_rectangle(0, 0, 54, 54,
    ...
    def _build_right_panel(self):
        ...
        eng_sb = tk.Scrollbar(self._cards_frame, orient="vertical",
        ...
        self._eng_inner.bind("<Configure>",
            lambda e: self._eng_canvas.configure(
                scrollregion=self._eng_canvas.bbox("all")))
    ...
    def _build_eng_card(self, eng):
        ...
        # ── Progress bar ──
        pbar_outer = tk.Frame(body, bg=C["border"], height=6)
        pbar_outer.pack(fill="x", pady=(6, 2))
        pbar_outer.pack_propagate(False)
        ...
        def _draw_bar(ev, outer=pbar_outer, pct=pct_clamp, col=bar_color, bg=card_bg):
            w = ev.width
            if w < 2:
                return
            filled = max(1, int(w * pct)) if pct > 0 else 0
            for ch in outer.winfo_children():
                ch.destroy()
            if filled:
                tk.Frame(outer, bg=col, width=filled, height=6).pack(side="left", fill="y")

        pbar_outer.bind("<Configure>", _draw_bar)
        ...
        for tab_key, tab_name in tab_defs:
            ...
            chip = tk.Frame(strip, bg=C["chip_active"],
                            padx=6, pady=3,
                            highlightthickness=1,
                            highlightbackground=dot_col if v > 0 else C["border"])
            chip.pack(side="left", padx=(0, 4), pady=1)
            tk.Frame(chip, bg=dot_col, width=6, height=6).pack(side="left", padx=(0, 4))
...
class HomeScreen(tk.Frame):
    ...
    def _build(self):
        ...
        btns.pack(fill="x")
        tk.Label(btns, text="GET STARTED", bg=C["sidebar"],
                 fg=C["muted"], font=("Segoe UI", 8, "bold")).pack(anchor="w", pady=(0, 8))
        ...
        r_sb = tk.Scrollbar(r_outer, orient="vertical", command=r_cv.yview)
        r_cv.configure(yscrollcommand=r_sb.set)
        r_sb.pack(side="right", fill="y")
        r_cv.pack(side="left", fill="both", expand=True)
        r_inner = tk.Frame(r_cv, bg=C["bg"])
